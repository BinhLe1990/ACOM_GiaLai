...
{
    public class BSCRegistry
    {
        public static string Exx = "";
        static private string Encrypt(string clearText, string Password)
        {
        ...
        static private byte[] Encrypt(byte[] clearData, byte[] Key, byte[] IV)
        {
        ...
        static private string Decrypt(string cipherText, string Password)
        {
        ...
        static private byte[] Decrypt(byte[] cipherData, byte[] Key, byte[] IV)
        {
            MemoryStream ms = new MemoryStream();
            Rijndael alg = Rijndael.Create();
            alg.Key = Key;
        ...
        {
            Microsoft.Win32.RegistryKey key = Microsoft.Win32.Registry.CurrentUser.OpenSubKey("Software\\" + Properties.Settings.Default.Registry_Name + "\\EMIS_NET\\" + subKey, true);
            try
            {
                if (key == null) //If the key doesn't exist.
                    key = Microsoft.Win32.Registry.CurrentUser.CreateSubKey("Software\\" + Properties.Settings.Default.Registry_Name + "\\EMIS_NET\\" + subKey);
                //Set the value.
        ...
        public static bool ReadRegistry(String subKey, String GetValueName, out Object ReturnValue)
        {
            ...
            try
            {
                key = Microsoft.Win32.Registry.CurrentUser.OpenSubKey("Software\\"+Properties.Settings.Default.Registry_Name+"\\EMIS_NET\\" + subKey, true);
                if (key == null)//If the key doesn't exist.
                {
