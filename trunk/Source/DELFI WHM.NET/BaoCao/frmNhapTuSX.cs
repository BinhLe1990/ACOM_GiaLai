...
{
    public partial class frmNhapTuSX : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void Search()
        {
            ...
            griDanhSach.DataSource = cnn.SQL_GetStoredProcedure("SP_BC_NhapTuSX", Thamso);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (dt != null)
            {
                BaoCaoTK.frmViewReport frm = new DELFI_WHM.NET.BaoCaoTK.frmViewReport(Application.StartupPath + @"\Report\BC_NhapTuSX.repx", dt);                
                frm.Show();                
            }
        }

        private void frmNhapTuSX_Load(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                using (DBACOMEntities db = new DBACOMEntities())
                {
                    ...
                    if (cnn.DT_DataTable("SELECT * FROM His_Export_NhapXuatSX WHERE " +
                                        " Ngay = '" + Convert.ToDateTime(dtpNgay.uValue).ToString("yyyy-MM-dd") + "' " +
                                        " AND Ca = '" + cboShift.uEditValue + "' AND Type = 'NhapTuSX'").Rows.Count > 0)
                    {
                    ...
                    else
                    {
                        ...
                        dt = cnn.SQL_GetStoredProcedure("SP_EX_NhapTuSX", Thamso);
                        if (dt.Rows.Count == 0)
                        {
